...
export const CustomParticipantSelector = ({
  value,
  onChange,
  participantData,
}) => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} className="w-[300px]">
      <PopoverTrigger asChild>
        ...
        >
          {value
            ? participantData.find((participant) => participant.value === value)
                ?.label
            : "Select Team/participant"}
...
export const CreateResultPage = () => {
  ...
  const [rows, setRows] = useState([
    { participant: "", rank: "", score: "", description: "" },
  ]);
  ...
  const reStructureParticipant = () => {
    if (allEventParticipantsData.length > 0) {
      const restructuredParticipants = allEventParticipantsData.map(
        (participant) => ({
  ...
  return (
    <div className="flex flex-col w-11/12 mx-auto items-center mt-2 text-center min-h-screen">
      <h1 className="text-3xl text-white font-bold mb-8 mt-4">
        Create Result by Event
      </h1>
          ...
          <div key={index} className="flex flex-row gap-3 mt-2 mb-2">
            <CustomParticipantSelector
              ...
              onChange={(selectedParticipant) =>
                handleInputChange(index, "participant", selectedParticipant)
              }
            ...
            />
            <Button onClick={() => handleDeleteRow(index)} className="ml-2">
      ...
      {round && participantData.length > 0 && (
        <>
          <Button onClick={handleAddRow} className="mt-5 mb-4">
            <Plus className=" text-green-400" />
          </Button>
          <Button onClick={handleSubmit} className="mt-10 mb-10">
            Submit
          </Button>
        </>
      )}
