...
export async function POST(req) {
  ...
  try {
    ...
    if (existingUser) {
      return NextResponse.json({
        success: false,
        message: "Hospitality User already exists",
      });
    }
    ...
    const user = await User.create({
      name,
      festId: "TCO113" + mobile.substring(0, 7),
      mobile: "003" + mobile.substring(0, 7),
    ...
    return NextResponse.json({
      success: true,
      message: "Hospitality User Registered Successfully!",
      data: user,
    });
  } catch (error) {
    return NextResponse.json({
      error: err.message,
      success: false,
      message: "Unable to Create Hospitality User",
    });
  }
}
