...
const CertDownloader = ({ user }) => {
  ...
  const generatePDF = async (name) => {
    try {
      ...
      const certificateIdTextWidth = ProductSansFont.widthOfTextAtSize(
        certificateIdText,
        10
      );
      const certificateIdTextX = qrCodeX - 8;
      const certificateIdTextY = qrCodeY - 12;
      ...
      const textWidth = ProductSansFont.widthOfTextAtSize(name, 22);

      const x = (width - textWidth - 10) / 2;
      const y = (height - 120) / 2;
      ...
      var file = new File(
        [pdfBytes],
        "TechFusionParticipationCertificate.pdf",
        {
  ...
  return (
    <div>
      <Button
        ...
        className="z-20 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 relative rounded-2xl mb-2 mt-20 pt-2 pb-2 pr-4 pl-4 border border-transparent bg-gray-900 text-white px-5 py-2 hover:bg-purple-500 border-white hover:border-none flex items-center"
      >
